...
# Searching
res2 = ""  # Initialize res2 before the loop starts to avoid referencing an undefined variable
for img in glob.glob(dir + "/Search_Image/*.jpeg"):
    ...
    # Check if number_plate is None (i.e., no plate detected)
    if number_plate:
        res2 = str("".join(re.split("[^a-zA-Z0-9]*", number_plate)))
    else:
        res2 = ""  # In case no plate is detected, set res2 to an empty string

# Check if res2 is not empty before printing
if res2:
